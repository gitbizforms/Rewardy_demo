<?
    ...
    $sql = "select count(idx) as cnt from work_page_error_list ";
    // $sql = $sql . = "where regdate >= '".$last_week."' and regdate <= '".$today."'";
    $history_info_cnt = selectQuery($sql);
	if($history_info_cnt['cnt']){ 
		$total_count = $history_info_cnt['cnt'];
	}
...
<html lang="en">
		...
		<div id="layoutSidenav">
            ...
            <div id="layoutSidenav_content">
                    <div class="container-fluid px-4" id="page-screen">
                        <h1 class="mt-4">에러 로그</h1><br>
                        <div class="card mb-4">
                            <div class="card-body">
								<input type="hidden" id="backoffice_type" value="backerror_log">
											...
											<div class="rew_member_sub_func_period">
                                                <div class="rew_list_char" id="rew_list_char">
													...
													<ul>
														...
														<li><button value="select"><span>select</span></button></li>
														<li><button value="insert"><span>insert</span></button></li>
														<li><button value="update"><span>update</span></button></li>
													</ul>
												</div>
                                ...
                                <table class="table table-bordered rounded-1 mt-3" id="backoff_table" class="rounded-1">
                                    <thead>
                                        <tr>
                                            <th class="backoff_erridx"><div class="back_sortkind" value="">No<button class="list_arrow" value="btn_sort_down"></button></div></th>
                                            <th class="backoff_query"><div class="back_sortkind" value="query">Query<button class="list_arrow" value="btn_sort_down"></button></div></th>
                                            <th class="backoff_errkind"><div class="back_sortkind" value="kind">종류<button class="list_arrow" value="btn_sort_down"></button></div></th>
                                            <th class="backoff_errpage"><div class="back_sortkind" value="page">발생페이지<button class="list_arrow" value="btn_sort_down"></button></div></th>
                                            <th class="backoff_errhost"><div class="back_sortkind" value="host">호스트<button class="list_arrow" value="btn_sort_down"></button></div></th>
                                            <th class="backoff_errdate"><div class="back_sortkind" value="regdate">발생시간<button class="list_arrow" value="btn_sort_down"></button></div></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?
                                            $sql = "select idx, regdate, code, query, kind, host, page from work_page_error_list";
                                            // $sql = $sql .= " where state = '0' and (regdate >= '".$last_week."' and regdate <= '".$today."')"; 
                                            $sql = $sql .= " order by idx desc limit ".$startnum.",".$pagesize;
	...
	<style>
		 .highlight {
            background-color: yellow; /* 배경색을 원하는 색으로 설정 */
         }
	</style>
</html>
