<?
	...
	//	$user_id="dasani003@nate.com";
	}

	//일일업무
	$sql = "select idx, state, work_flag, decide_flag, email, name, work_idx, repeat_work_idx, repeat_flag, notice_flag, share_flag, memo_view, contents_view, title, contents, workdate, date_format( regdate , '%m/%d/%y %l:%i:%s %p') as reg, date_format(regdate, '%H:%i') as his, party_link from work_todaywork where state !='9'";
	$sql = $sql .=" and idx = '64194' ";
	...
	//프로필사진
	//$profile_box_img_src = profile_img_info($user_id);
	...
	//예약업무 예약기능
	/*$sql = "select idx, title, type_flag from work_decide where state='0' and companyno='".$companyno."' and type_flag='0' order by sort asc";
	$decide_info = selectAllQuery($sql);
	if($decide_info['idx']){
		$decide_list = @array_combine($decide_info['idx'], $decide_info['title']);
	}*/
	...
	//정시 출근시간(09:00) 2회이상 일경우 코칭챌린지 표기
...
$(document).ready(function(){
	...
	$(document).on("click",".tuto_pop_03_01 .tuto_next", function(){
		$(".tuto_mark_03_01").hide();
        $(".tuto_pop_03_01").hide();

        $(".layer_reward").show();
        tuto_position();

        $(".tuto_mark_03_02").show();
        $(".tuto_pop_03_02").show();
	})
...
function tuto_position(){
    $(".tuto").each(function(i){
        ...
        var tt_r = win_w - 400;
        var tt_p = $(".tuto_pop_03_0"+i+"").height();
        var tt_ph = tt_p + tt_y;
        if(tt_x > tt_r){
            $(".tuto_pop_03_0"+i+"").css({
                left:"auto",
            ...
            $(".tuto_pop_03_0"+i+"").removeClass("tuto_l tuto_r").addClass("tuto_r");
        }else{
            $(".tuto_pop_03_0"+i+"").css({
                left:(tt_x-47),
                opacity:1
            });
            $(".tuto_pop_03_0"+i+"").removeClass("tuto_l tuto_r").addClass("tuto_l");
        }
        if(tt_ph > (win_h - 70)){
            $(".tuto_pop_03_0"+i+"").css({
                top:(tt_t-tt_p-24),
            });
            $(".tuto_pop_03_0"+i+"").removeClass("tuto_t tuto_b").addClass("tuto_t");
        }else{
            $(".tuto_pop_03_0"+i+"").css({
                top:(tt_y+42),
            });
            $(".tuto_pop_03_0"+i+"").removeClass("tuto_t tuto_b").addClass("tuto_b");
        }
        $(".tuto_mark_03_0"+i+"").css({
            left:tt_x,
            top:tt_y,
            opacity:1
        });
...
$(document).on("click",".tuto_pop_03_02 .tuto_prev",function(){
    $(".tuto_mark_03_02").hide();
    $(".tuto_pop_03_02").hide();
	$(".layer_reward").hide();

    $(".tuto_mark_03_01").show();
    $(".tuto_pop_03_01").show();
});

$(document).on("click",".tuto_pop_03_02 .tuto_next",function(){
	$(".layer_reward").hide();
    var fdata = new FormData();
    fdata.append("mode","update");
    fdata.append("level","coin");
...
<div class="rew_warp">
													...
													<?if($works_info['idx']){?>
														...
														<?for($i=0; $i<count($works_info['idx']); $i++){
															...
															<li class="tdw_list_li<?=$li_class?>" id="workslist_<?=$idx?>" value="<?=($i+1)?>">
																<div class="tdw_list_box<?=($state=='1')?" on":""?><?=$share_view_bt_style?>" id="tdw_list_box_<?=$idx?>">
																	<div class="tdw_list_chk">

																		<?if($work_flag=='1'){?>
																			<button class="btn_tdw_list_chk" <?=$tdw_list?" id='tdw_dlist_chk'":""?>><span>완료체크</span></button>
											
																		<?}else{?>
																			<button class="btn_tdw_list_chk" value="<?=$idx?>" <?=$tdw_list?" id='tdw_dlist_chk'":""?>><span>완료체크</span></button>
																		<?}?>
																	...
																	<div class="tdw_list_function">
																		<div class="tdw_list_function_in">
																			...
																			if($work_flag=="1" && $work_idx || $share_flag=='2' && $work_idx){?>
																				<button class="tdw_list_100c tuto tuto_03_01" title="100코인" id="tdw_list_100c" value="<?=$idx?>"><span>100</span></button>
																			<?}?>

																			<?if($notice_flag=='0' || $decide_flag=='0'){?>
																			...
																			if($work_flag=="1" && $work_idx){?>
																				<button class="tdw_list_reported_hart<?=$work_like_list[$work_idx]>0?" on":""?>" title="좋아요 " <?=$work_like_list[$work_idx]>0?"":" id=\"tdw_list_jjim_2\""?> value="<?=$work_idx?>"><span>좋아요</span></button>
																			<?
																			//공유받음
																			}else if($share_flag=='2' && $work_idx){?>
																				<button class="tdw_list_shared_hart<?=$work_like_list[$work_idx]>0?" on":""?>" title="좋아요" <?=$work_like_list[$work_idx]>0?"":" id=\"tdw_list_jjim_2\""?> value="<?=$work_idx?>"><span>좋아요</span></button>
																			<?}else{?>
	...
	<div class="tuto_phase_in">
		...
		<div class="tuto_phase_list tuto_clear">
			...
			<div class="tuto_phase_box phase_04 tuto_on">
				<p>4</p>
				<button onclick="location.href='/party/tu_project.php'">
					<dl>
						<dt>파티 체험</dt>
						<dd>
							<span>역량</span>
		...
		<div class="tuto_phase_pause">
			<button onclick="location.href='/team/index.php'">다음에 이어하기</button>
		</div>
	</div>
</div>
	<?php
	...
	<?php
		//튜토리얼 가이드 레이어
		include $home_dir . "/layer/tutorial_coin_layer.php";
	?>

	<?php
		//튜토리얼 페이즈 레이어
		//  include $home_dir . "/layer/tutorial_phase.php";
	?>
	...
	<?php
		//코인보상하기 레이어
		include $home_dir . "/layer/member_tu_reward.php";
	?>
...
<?
	...
	//work_cp_reward_plus("cp", "0007", '123456', $user_id, "reward");

	//레퍼러체크
	// if($_SERVER['HTTP_REFERER']){
	// 	//이전페이지가 메인페이지 일때
	// 	if(strpos($_SERVER['HTTP_REFERER'], "/team/") !== false) {?>
	// 		<script>
	// 			var read_date = '<?=$_COOKIE['read_date']?>';
	// 			$("#work_wdate").val(read_date);
	// 			date_change();
	// 			works_list();
	// 		</script>
	// 	<?
	...
	// 	work_read_check($user_id, "day", $_COOKIE['read_date'], "");
	// }else{
	// 	work_read_check($user_id, "day", $sel_wdate, "");
	// }

	// if($_COOKIE['read_date']){
	// 	unset($_COOKIE['read_date']);
	// }
