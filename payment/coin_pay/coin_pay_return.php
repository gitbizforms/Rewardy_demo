...
include FUNC_MYSQLI;
        ...
        require_once('pay_libs/HttpClient.php');
        require_once('pay_libs/properties.php');
            ...
            if (strcmp("0000", $_REQUEST["resultCode"]) == 0) {
                ...
                if (strcmp($authUrl, $_REQUEST["authUrl"]) == 0) {
                    ...
                    try {
                        ...
                        $chargePrice = $resultMap['TotPrice'] * 20;
                        $sql = "insert into payment_ge(companyno,payMethod,resultCode,resultMsg,tid,mid,MOID,
                                 TotPrice,goodName,applDate,applTime,buyerName,buyerEmail,buyerTel,custEmail) 
                                values('".$merchantData."', '".$resultMap['payMethod']."','".$resultMap['resultCode']."','".$resultMap['resultMsg']."','".$resultMap['tid']."','".$resultMap['mid']."','".$resultMap['MOID']."',
                                '".$resultMap['TotPrice']."','".$resultMap['goodName']."','".$resultMap['applDate']."','".$resultMap['applTime']."','".$resultMap['buyerName']."','".$resultMap['buyerEmail']."','".$resultMap['buyerTel']."','".$resultMap['custEmail']."')";
                                $pay_in = insertIdxQuery($sql);     
                        if($pay_in){
                            $sql = "select idx,company,comcoin from work_company where state = '0' and idx = '".$merchantData."'";
                            $work_check = selectQuery($sql);
                            if($work_check){
                               $sql = "update work_company set comcoin = ".$work_check['comcoin']." + ".$chargePrice." where idx = '".$work_check['idx']."'";  
                               $charge_com = insertIdxQuery($sql);
                            }

                            $sql = "insert into payment_log(state,companyno,payMethod,resultCode,resultMsg,tid,mid,MOID,
                                    TotPrice,goodName,applDate,applTime,buyerName,buyerEmail,payDevice) 
                                    values('0','".$merchantData."', '".$resultMap['payMethod']."','".$resultMap['resultCode']."','".$resultMap['resultMsg']."','".$resultMap['tid']."','".$resultMap['mid']."','".$resultMap['MOID']."',
                                    '".$resultMap['TotPrice']."','".$resultMap['goodName']."','".$resultMap['applDate']."','".$resultMap['applTime']."',
                                    '".$resultMap['buyerName']."','".$resultMap['buyerEmail']."','PC')";
