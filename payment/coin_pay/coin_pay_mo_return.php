<?php
   ...
   if ($_REQUEST["P_STATUS"] === "00") {             // 인증이 P_STATUS===00 일 경우만 승인 요청
        ...
        if (strcmp($P_REQ_URL, $_REQUEST["P_REQ_URL"]) == 0) {
            ...
            $date_part = substr($out['P_AUTH_DT'], 0, 8);
            $time_part = substr($out['P_AUTH_DT'], 8); 
            $price = $out['P_CARD_APPLPRICE'] * 20;
            $sql = "insert into payment_ge(companyno,payMethod,resultCode,resultMsg,tid,mid,MOID,TotPrice,goodName,applDate,applTime) 
            values('".$P_NOTI."','".$out['P_TYPE']."','".$out['P_STATUS']."','".$out['P_RMESG1']."','".$out['P_TID']."','".$out['P_MID']."','".$out['P_MID']."_mobile','".$out['P_CARD_APPLPRICE']."','코인 충전','".$date_part."','".$time_part."')";
            $coin_pay = insertIdxQuery($sql);
            
            if($coin_pay){
                // 회사 코인 업데이트
                $sql = "select idx,company,comcoin from work_company where idx = '".$P_NOTI."' and state = '0'";
                $char_com = selectQuery($sql);
                if($char_com){
                $sql = "update work_company set comcoin = ".$char_com['comcoin']." + ".$price." where idx = '".$char_com['idx']."'";
                $up_com = updateQuery($sql);
                }
                $sql = "insert into payment_log(state,companyno,payMethod,resultCode,resultMsg,tid,mid,MOID,payMethodDetail,
                                    TotPrice,goodName,applDate,applTime,payDevice) 
                                    values('0','".$P_NOTI."', '".$out['P_TYPE']."','".$out['P_STATUS']."','".$out['P_RMESG1']."','".$out['P_TID']."','".$out['P_MID']."','".$out['P_MID']."_mobile','SETTING',
                                    '".$out['P_CARD_APPLPRICE']."','코인 충전','".$date_part."','".$time_part."'
                                    ,'Mobile')";
    ...
    <body class="wrap">
        ...
        <main class="col-8 cont" id="bill-01">
            ...
            <section class="menu_cont mb-5">
				...
				<button onclick="location.href='pay_pop_return_mo.php'" class="btn_solid_pri col-6 mx-auto btn_lg" style="margin-top:50px">돌아가기</button>
