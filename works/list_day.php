	...
	<?for($i=0; $i<count($res['idx']); $i++){
		...
		//$contents = iconv("UTF-8", "utf8_general_ci", $contents); 
			...
			if($state == 0){
				...
				$cls ="";
				$span_btn = "<button id=\"list_complete_".$idx."\" value=\"".$idx."\"><span>완료하기</span></button>";
				$span_btn .= "<button id=\"list_del_".$idx."\" value=\"".$idx."\"><span>삭제하기</span></button>";
				$span_btn .= "<button id=\"list_yesterday_".$idx."\" value=\"".$idx."\"><span>내일로 미루기</span></button>";

			}else if($state == 1){

				$cls =" class=\"on\"";
				$span_btn = "<span>완료하기</span>";
				$span_btn .= "<button id=\"list_del_".$idx."\" value=\"".$idx."\"><span>삭제하기</span></button>";
				$span_btn .= "<span>내일로 미루기</span>";
			}


			//로그인아이디와 작성자아이디가 같은경우(수정 및 삭제 권한부여)
			if($user_id == $list_id){
				$list_editor = true;
			}else{
				$list_editor = false;
		...
		?>
			<li <?=$cls?>>
				<input type="hidden" id="listidx" value="<?=$idx?>">
				<div class="tc_chk">
					<?if($state == 1){?>
						<button id="list_recomplete_<?=$idx?>" value="<?=$idx?>"><span>완료하기</span></button>
					<?}else{?>
						<button id="list_complete_<?=$idx?>" value="<?=$idx?>"><span>완료하기</span></button>
					<?}?>
				</div>
				<div class="tc_desc" <?if($list_editor == true){?>id="tc_dec_<?=$idx?>"<?}?>>
					<span <?if($list_editor == true){?>id="contents1_<?=$idx?>"<?}?>><?=$contents?></span>
					<div class="tc_request">
						<?if($work_flag == '1'){?>

							<span>업무예약</span>
							<input id="date1" value="<?=$workdate?>" size="8" maxlength="10">

						<?}else if($work_flag == '2'){?>
							
							<span>업무요청</span>
							<strong id="requser_<?=$idx?>" value="<?=$idx?>"><?=$from_name?></strong>

						<?}else if($work_flag == '3'){?>

							<span id="contents2_<?=$idx?>"><?=$contents1?></span>
							<br>
							<span>목표</span>
							<input id="date1" value="<?=$req_date?>" size="5" maxlength="10" style="border: 0px solid #2196F3;">
						<?}?>

					</div>
				</div>

				<?if($work_flag == '2' && $list_editor == true){?>
					<div class="tc_request_user">
						<div class="tc_request_user_in">
							<div>
								<input type="checkbox" name="chkuseall<?=$idx?>" id="chkuseall<?=$idx?>" value="all" />
								<label for="chkuseall<?=$idx?>">전체선택</label>
							</div>

							<?for($j=0; $j<count($mem_req['idx']); $j++){
								
								if(in_array($mem_req['idx'][$j], $req_user)){
							...
							?>
								<div>
									<input type="checkbox" name="requsechk<?=$idx?>" id="requsechk<?=$idx?>_<?=$j?>" value="<?=$mem_req['idx'][$j]?>" <?=$chkd?>/>
									<label for="requsechk<?=$idx?>_<?=$j?>"><?=$mem_req['name'][$j]?><span>(<?=$mem_req['part'][$j]?>)</span></label>
								</div>
							<?}?>
							
						</div>
						<div class="tc_request_user_btn"><button id="req_use_edit" value="<?=$res['idx'][$i]?>"><span>확인</span></button></div>
					</div>
				<?}?>

				<div class="tc_function">
					<button class="tc_function_open"><span>&middot;<br />&middot;<br />&middot;</span></button>
					<div class="tc_function_list">
						<?=$span_btn?>
					</div>
				</div>

				<div class="tc_modify" id="edit_content1_<?=$idx?>" value="<?=$idx?>" style="display:none;">
					<textarea name="contents1_<?=$idx?>" id="contents1_<?=$idx?>" class="area_02"><?=strip_tags($contents)?></textarea>
					<button id="edit" value="<?=$idx?>"><span>수정</span></button>
				</div>

				<div class="tc_modify" id="edit_content2_<?=$idx?>" value="<?=$idx?>" style="display:none;">
					<textarea name="contents2_<?=$idx?>" id="contents2_<?=$idx?>" class="area_02"><?=strip_tags($contents1)?></textarea>
					<button id="edit" value="<?=$idx?>"><span>수정</span></button>
				</div>
			</li>
		<?}?>
