<?
	...
	if($party_idx){
		...
		$sql .= " where a.state!='9' and b.state='0' and a.companyno='".$companyno."' and a.idx='".$party_idx."' and c.state != '9'";
		$sql .= " order by";
		...
		//좋아요수
		$sql = "select count(1) as cnt from work_todaywork_like where state='0' and companyno='".$companyno."' and work_idx='".$party_idx."' and service='project'";
...
<div class="t_layer rew_layer_character item_prof" style="display:none;">
		...
		<div class="tl_in">
			...
			<div class="tl_profile">
				<ul>
					<?for($i=0; $i<count($character_head_img_info['idx']); $i++){
						$idx = $character_head_img_info['idx'][$i];
						$file_path = $character_head_img_info['file_path'][$i];
						$file_name = $character_head_img_info['file_name'][$i];
						$fp_flag = $character_head_img_info['fp_flag'][$i];
							...
							<div class="tl_profile_box">
								<div class="tl_profile_img" style="background-image:url(<?=$character_img_src?>);">
									<?if($fp_flag == 0 || $img_buy_arr[$idx] != ''){?>
										<button class="btn_profile<?=$member_row_info_my['profile_type']=='0' && $member_row_info_my['profile_img_idx']==$idx?" on":""?>" id="profile_img_0<?=$idx?>" value="<?=$idx?>"><span>기본 프로필 이미지1 선택</span></button>
									<?}else{?>
