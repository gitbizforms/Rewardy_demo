<?
	...
	if($coaching_chk){
		//조건절
		$where = "where a.state='0' and view_flag='0'";
	}else{
		//조건절
		$where = "where a.state='0' and coaching_chk='0'";
	}
	...
	//챌린지 리스트
	$sql = "select * from (";
	$sql = $sql .=" select ROW_NUMBER() over(order by a.idx desc) as r_num, a.idx, a.state, a.day_type, a.attend_type, attend, a.cate, a.title, a.company, a.email, a.keyword, a.sdate, a.edate, DATEDIFF(DD, convert(varchar(10), getdate(), 120), a.edate ) as chllday, a.temp_flag,";
	...
	$sql = $sql .=" group by a.idx, a.state, a.attend_type, a.cate, a.title, a.coin, a.company, a.email, a.day_type, attend, a.temp_flag, a.keyword, a.sdate, a.edate, DATEDIFF(DD,a.sdate, a.edate)";
	...
	if($chall_info['idx']){
		...
		$html = $html .='		<strong><span>임시저장된 챌린지가 없습니다.</span></strong>';
		$html = $html .='	</div>';
