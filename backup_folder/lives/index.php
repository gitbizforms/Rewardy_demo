<?
	...
	$newUrl = "https://rewardy.co.kr/live/index.php";
	...
	if($check_zzim){
		...
		$where_zzim = "and (c.email = '".$user_id."' or c.email != '')";
	}
	//메인 전체 회원 리스트
	...
	// 업무일정
	$sql = "select idx, email, work_flag, decide_flag, work_stime, work_etime from work_todaywork use index(state) where state='0' and companyno='".$companyno."' and share_flag!='2' and decide_flag != '0' and workdate='".TODATE."' group by email, state, work_flag, decide_flag";
	...
	//완료한 업무 조회
	$sql = "select a.idx, a.email, b.name, a.work_flag, a.decide_flag, a.work_idx, a.share_flag, a.secret_flag, a.title, a.work_stime, a.work_etime, a.contents, a.editdate, a.regdate from work_todaywork as a use index(state) left join work_member as b on(a.email=b.email)";
	...
	//오늘 마음전하기
	$sql = "select email, name, count(1) as cnt from work_todaywork_like where state='0' and service='lives' and like_flag='0' and workdate='".TODATE."' group by email, name limit 1";
...
<div class="rew_warp live_warp">
									...
									<div class="rew_live_my_left">
										...
										<div class="live_tab">
											<div class="live_tab_in">
												...
												<input type="hidden" id="user_email" value="all">
												<ul>
													<li class="on option_count" id="lives_list_cnt" value = "all"><span><?=$member_live_cnt?><em>명</em></span><em>정상근무</em></li>
																...
																<div class="ldr_user_in">
																	...
																	<div class="ldr_user_style">
																		<div class="ldr_user_img">
																			...
																			<div class="ldr_user_imgs" id="ldr_user_imgs_<?=$member_list_idx?>" style="background-image:url('<?=$profile_file?$profile_img:"/html/images/pre/img_prof_default.png"?>');"></div>
	...
	<script src= "/lives/js/lives.js"></script>
	<!-- Step 1) Load D3.js -->
