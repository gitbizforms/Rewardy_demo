...
$sql = "select idx, state, contents, editdate, title from bro_manual where idx = '".$idx."' ";
$page = selectQuery($sql);

$sql = "select idx, state, title from bro_manual where idx < '".$idx."' and state = '1' order by idx desc limit 0,1";
$prev = selectQuery($sql);

$sql = "select idx, state, title from bro_manual where idx > '".$idx."' and state = '1' order by idx asc limit 0,1";
			...
			<div class="notice_wrap">
						...
						<div class="notice_view_move">
							...
							<div class="notice_view_next">
								<button class="notice_<?=$next['idx']?>">
									...
									<span><?=$next['title']?$next['title']:'다음글이 없습니다.'?></span>
								</button>
							</div>
						</div>
					</div>
				...
				<div class="notice_bottom">
					<div class="notice_btn">
						<button onclick="location.href='/about/customer/manual.php'"><span>목록</span></button>
					</div>
				</div>
			</div>
		</div>
    ...
    <script type="text/javascript">
			//이전글, 다음글
  			$(document).on("click","button[class^=notice_]", function(){
				...
				location.href = "manual_view.php?idx="+no;
			});
    </script>
